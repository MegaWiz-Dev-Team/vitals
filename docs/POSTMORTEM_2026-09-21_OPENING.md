...
- ~~The ops service account cannot yet submit Cloud Build.~~ Closed 22 Sep. Two things were wrong, neither of them propagation: `gcloud builds submit` lists the project's buckets to check the staging bucket's ownership, which needs project-level `storage.bucketViewer` (bucket-level admin does not cover it); and once the build ran, gcloud exited 1 because the account cannot stream the build log, with the build green — so the deploy script now reads the build's status by id and treats gcloud's exit code as advisory. First deploys as the account: staging 00075, production 00027.
...
|---|---------|--------------------------|--------|
| 1 | A person's login was the only way to open the door | `scripts/ward-door.sh open\|preview\|closed` flips the door as the ops service account, from the secret, with no build — and refuses to run as a person. The scheduled tick and the factory already run as the service account. | `scripts/ward-door.sh` done 22 Sep (branch `cwf/ops`, 19 cases in its harness, run by gates; first real flips on staging 00074–00077 as the service account). **Closed on the real path the same night:** staging 00075 and production 00027 were built and deployed end to end as the service account — builds 3d51fd6a and 0f4d83b6, status read from the build itself — with no person's login anywhere in the path. |
